...
// Makro do sprawdzania b³êdów CUDA
...
// Stan pocz¹tkowy poddrzewa, od którego w¹tek zaczyna DFS
struct GPUState {
    int idx;
    int value;
    int weight;
...
// Górne oszacowanie (relaksacja u³amkowa)
...
// Ka¿dy w¹tek przeszukuje swoje poddrzewo metod¹ DFS na lokalnym stosie
__global__ void bnbKernel(
    ...
    while (sp > 0) {
        ...
        // Co jakiœ czas odœwie¿amy lokalny best, ¿eby uwzglêdniaæ postêpy innych w¹tków bez ci¹g³ych odczytów funkcji atomowych
        if ((sp & 7) == 0) {
            localBest = atomicAdd(globalBest, 0);
        }
        ...
        // Ochrona przed przepe³nieniem stosu
        if (sp + 2 > 32) continue;

        // Rozwiniêcie wêz³a 1) ga³¹Ÿ nie bierzemy wrzucamy jako pierwsz¹, 2) ga³¹Ÿ bierzemy jako drug¹
...
int solveGPU(const ProblemData& data) {
    ...
    // G³êbokoœæ prefiksu generowanego na CPU (liczba stanów startowych = 2^prefixDepth)
    int prefixDepth = 9;
    ...
    // Inicjalizacja zach³anna jako dolne ograniczenie
    ...
    // Generowanie stanów startowych + odcinanie ju¿ na CPU
    ...
    for (int mask = 0; mask < totalMasks; mask++) {
        ...
        // Jeœli  bound nie przebija greedy, nie wysy³a tego na GPU
        if (ub > greedyBest) {
    ...
    // Uruchamianie kernela w batchach zamiast ucinaæ liczbê bloków przetwarzamy wszystkie stany porcjami
